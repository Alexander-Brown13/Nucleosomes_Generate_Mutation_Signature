...
def saddle_up(dyad_file):
    files_1hr_XR = get_file_list(r"I:\Melanoma_Fixing_1bp_Error\XRseq\BEDs\4hr")
    dir_0hr_by_80J = r"I:\Melanoma_Fixing_1bp_Error\XRseq\1hr\Transcript_Nucleosome_Tallies"
    files_0hr_by_80J = get_file_list(dir_0hr_by_80J)

    for les_file in files_0hr_by_80J:
        start_search_string = "NoOverlaps_"
        start_ID_index = find_nth_occurrence_rindex(les_file, start_search_string, 1) + len(start_search_string)
        end_ID_index = find_nth_occurrence_rindex(les_file, "_Transcripts", 1)
        unique_ID = les_file[start_ID_index:end_ID_index]
        if unique_ID in dyad_file:
            file_0hr_by_80J = les_file
            break
    ...
    tally_dict_80J = get_80J_tally_dict(file_0hr_by_80J)

    outfile_directory = r"I:\Melanoma_Fixing_1bp_Error\XRseq\4hr\Transcript_Nucleosome_Tallies"
    directory_index = find_nth_occurrence_rindex(dyad_file, "/", 1)
    extension_index = find_nth_occurrence_rindex(dyad_file, ".", 1)
    outfile_suffix = dyad_file[directory_index+1:extension_index]

    outfile = open(outfile_directory + "\\" + outfile_suffix + "_4hr_XR_by_Damage_Seq_Strong_Dyad_Lesion_Tallies.txt", "w")
    header = '\t'.join(["Bin", "4hr_XR_Lesion_Tally", "DMG_Lesion_Tally", "4hr_XR_by_DMG_Tally"])
    ...
    while i < len_of_dict+1:
        ...
        try:
            normalized_value = count_1hr_XR/count_80J
        except ZeroDivisionError:
            normalized_value = 0
        new_line = '\t'.join([str(s) for s in [i, count_1hr_XR, count_80J, normalized_value]])
        outfile.write(new_line + '\n')
        i += 1

if __name__ == '__main__':
    dyad_directory = r"I:\Melanoma_2018_Nuc_Project\Transcript_Sorted_Nucleosomes"
    dyad_files = get_file_list(dyad_directory)

    jobs = []
    for dyad_file in dyad_files:
        p = multiprocessing.Process(target=saddle_up, args=(dyad_file, ))
        jobs.append(p)
        p.start()
