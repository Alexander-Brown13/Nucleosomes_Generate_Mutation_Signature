...
def work_horse(mut_file, dyad_dict, mut_tally_dict, dinuc_tally_dict):
    ...
    for line in mut_reader:
        ...
        if len(dyad_list) == 0: # it is currently empty
            continue
        mut_list = [int(line[1]), int(line[2])]
#        mut_pos = int(line[1]) - 1
        lesion_count = 1

        for mut_pos in mut_list:
...
#    chr_dict = generate_chr_dict()
    ...
    files_0hr = get_file_list(r"I:\Melanoma_Fixing_1bp_Error\HS_DMGseq\BEDs\0hr\CC_TC_CT")
    files_80J = get_file_list(r"I:\Melanoma_Fixing_1bp_Error\HS_DMGseq\BEDs\20J\CC_TC_CT")
    ...
    outfile = open("0hr_by_20J_Strong_Dyad_HS_DMG_Lesions_And_Dinuc_Tallies_CC_TC_CT.txt", "w")
    nuc_count = retrieve_nuc_count(dyad_dict)
    header = '\t'.join(["Bin", "0hr_Lesion_Tally", "20J_Lesion_Tally", "0hr_by_20J_Tally", "Nuc_Count:", str(nuc_count)])
