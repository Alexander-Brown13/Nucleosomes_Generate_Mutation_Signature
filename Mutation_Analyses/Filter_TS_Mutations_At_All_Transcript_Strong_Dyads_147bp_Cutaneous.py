...
def work_horse(mut_file, dyad_dict, chr_dict, mut_tally_dict, dinuc_tally_dict, outfile):
    ...
    for mut_line in mut_reader:
        ...
        if chromosome == "chrM_": # it is currently empty
            continue
...
def different_horse(mut_files, dyad_dict, chr_dict, dyad_file):
    ...
    trinuc_file = r"Cutaneous_simple_All_Trinuc_Tallies.txt"
    trinuc_freq_dict = generate_trinuc_freq_dict(trinuc_file)

    outfile_directory = r"I:\Melanoma_2018_Nuc_Project\Transcript_Nucleosome_Tallies"
    ...
    outfile_prefix = example_file[directory_index+1 : fourth_underscore_index]
    ...
    outfile = open(outfile_directory + "\\" + outfile_prefix + "_" + outfile_suffix + "_Strong_Dyad_NTS_Mutations.txt", "w")

    for mut_file in mut_files:
        work_horse(mut_file, dyad_dict, chr_dict, mut_tally_dict, dinuc_tally_dict, outfile)
...
def saddle_up():
    ...
    cC_mut_file = r"I:\Melanoma_2018_Nuc_Project\TS_and_NTS_Mutations\Cutaneous_Low_CCDS_Mutations_NTS.txt"
    cutaneous_mut_files = [cC_mut_file]

    for dyad_file in dyad_files:
        dyad_dict = generate_dyad_dict(dyad_file)
        different_horse(cutaneous_mut_files, dyad_dict, chr_dict, dyad_file)
