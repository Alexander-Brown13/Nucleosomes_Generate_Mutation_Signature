...
def work_horse(mut_file, dyad_dict, chr_dict, mut_tally_dict, dinuc_tally_dict, random_number_list):
    ...
    i = 0
    max_value = max(random_number_list)
...
def different_horse(mut_files, dyad_dict, chr_dict, i):
    ...
    out_dir = r"I:\Melanoma_2018_Nuc_Project\1000_Subsets"
    outfile = open(out_dir + "\\Subset_" + str(i) + "_Strong_Dyad_Mutation_And_Dinuc_Tallies.txt", "w")
    ...
    for mut_file in mut_files:
        if "C" in mut_file[find_nth_occurrence_rindex(mut_file, "_", 4):]:
            max_range = 1053197 #for cutaneous
            random_number_list = random.sample(range(1, max_range), 12700)
        if "T" in mut_file[find_nth_occurrence_rindex(mut_file, "_", 4):]:
            max_range = 117022 #for cutaneous
            random_number_list = random.sample(range(1, max_range), 1300)

        work_horse(mut_file, dyad_dict, chr_dict, mut_tally_dict, dinuc_tally_dict, random_number_list)
...
def saddle_up():
    ...
    i = 1
    out_num = 1
    while i <= subsets/cores_used:
        j = 1
        jobs = []
        while j <= cores_used:
            p = multiprocessing.Process(target=different_horse, args=(cutaneous_mut_files, dyad_dict, chr_dict, out_num))
            jobs.append(p)
            p.start()
            j += 1
            out_num += 1
